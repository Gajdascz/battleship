...
describe('Advanced AI', () => {
  ...
  describe('Initialization', () => {
    it('Should initialize fleet list correctly', () => {
  ...
  it('Should wrap coordinates helper', () => {
  ...
  it('Should initialize methods and properties', () => {
  ...
  describe('Move generation and finding', () => {
    describe('Finding the best move in correct logical order', () => {
      it('Should take element from priority queue when not empty', () => {
        ai.priorityMoves.enqueue([0, 0]);
        expect(ai.priorityMoves.isEmpty()).toBe(false);
        expect(ai.findBestMove()).toEqual([0, 0]);
        expect(ai.priorityMoves.isEmpty()).toBe(true);
      });
      it('Should find random open move around when ai has last hit and one in chain', () => {
        ai.lastHit = [1, 1];
      ...
      it('Should properly find the next move to continue hit chain', () => {
  ...
  describe('Move Processing', () => {
    ...
    it('Should throw error when given an invalid move', () => {
      expect(() => ai.processMoveResult(null, '')).toThrowError();
    });
    it('Should update last hit, hit chain and total hits on hit', () => {
      ai.processMoveResult([0, 0], RESULTS.HIT);
      assertProperties([0, 0], 1, 0, 0, 1);
    });
    it('Should update last hit to null, total sunk, resolved hits,and destruct hit chain on ship sunk', () => {
      ai.lastSunkLength = 2;
      ai.processMoveResult([0, 0], RESULTS.HIT);
    ...
    describe('Unresolved Chain Processing', () => {
      it('Should return resolved hits equal to ship sunk length and unresolved equal to remainder', () => {
        ai.lastSunkLength = 3;
