...
export default function ElementManager() {
  ...
  const getUnexploredCells = (board) =>
    board.querySelectorAll('button.grid-cell[data-cell-status="unexplored"]');

  const getMainFleetList = (board) => board.querySelector('.main-fleet-list');
  ...
  return {
    ...
    isEmpty: () => elementCache.size === 0,
    reset: () => clean(elementCache),
    getUnexploredCells,
    getMainFleetList
  };
}
