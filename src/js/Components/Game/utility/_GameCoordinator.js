...
export const GameCoordinator = (emitterBundle) => {
  ...
  const CombatState = () => {
    const { emitter, getPlayerEventKey } = emitterBundle;
    const state = BaseState();
    ...
    const COMBAT_METHODS = {
      ...
      HAS_LOST: getPlayerEventKey('hasLost')
    };
    implementStateBase({
      state,
      callbacks: {
    ...
    let onHasLost = null;
    const setOnHasLost = (callback) => {
      if (onHasLost) removeOnHasLost(onHasLost);
      emitter.subscribe(COMBAT_METHODS.HAS_LOST, callback);
      onHasLost = callback;
    };
    const removeOnHasLost = (callback) => {
      if (!onHasLost) return;
      emitter.unsubscribe(COMBAT_METHODS.HAS_LOST, callback);
      onHasLost = null;
    };
